...
def _launch_kwargs() -> dict:
    """Watch-mode toggle (Browser-skill extension; orchestrator untouched).

    Set S9_BROWSER_HEADFUL=1 to open a REAL Chrome window with slow_mo so a
    human (or a screen recorder) can watch the search / filter / sort / click
    happen live. Default stays headless for normal runs and CI.
    S9_BROWSER_SLOWMO_MS tunes the per-action pause (default 800ms).
    """
    headful = os.environ.get("S9_BROWSER_HEADFUL", "").lower() in ("1", "true", "yes")
    if headful:
        slow = float(os.environ.get("S9_BROWSER_SLOWMO_MS", "800"))
        return {"headless": False, "slow_mo": slow}
